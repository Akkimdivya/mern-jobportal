        ...
        <div className='grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-12 my-12 items-center'>
          {
            salary.map((data)=>(
              <div key={data.id} className='shadow px-4 py-8'>
                ...
                <p className='my-2 font-medium text-primary text-lg'>{data.salary}</p>
                <div className='flex flex-wrap gap-4'>
                  <a href='/' className='underline text-black'>{data.status}</a>
                  <a href='/' className='underline text-black'>{data.skills}</a>
                </div>
              </div>
            ))
          }
